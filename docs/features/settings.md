...
> **All three intake settings are now consumed (features 040 + 041)**: `intakeWindow` drives the auto-miss engine (`ReconcileMissedIntakes`, `lib/features/meds/domain/usecases/reconcile_missed_intakes.dart` — feature 040) AND the Today screen's per-dose window classification; `gracePeriod` drives `UndoIntake`'s live undo window (replacing the old hardcoded `kIntakeUndoGracePeriod`); `allowMarkAhead` gates whether a future dose's checkbox is enabled before its window opens. All three reach the meds feature through a single reactive projection provider, `todayIntakeSettingsProvider` (`lib/features/meds/presentation/providers/intake_providers.dart`), so a change here updates the Today checklist live. See [`meds.md`](meds.md#today-screen--intake-logging-feature-038) and [`meds.md`](meds.md#auto-miss-engine-feature-040).
...
- [`../../specs/039-intake-settings/spec.md`](../../specs/039-intake-settings/spec.md) — the spec that added `intakeWindow`, `gracePeriod`, `allowMarkAhead`, the `IntakeWindow`/`GracePeriod` value objects, and the Intake settings section (foundation only at the time — all three are now consumed, see above)
...
- [`../../specs/041-today-redesign/spec.md`](../../specs/041-today-redesign/spec.md) — the spec that made `gracePeriod` and `allowMarkAhead` load-bearing: the redesigned Today screen reads all three settings live via `todayIntakeSettingsProvider`
- [`meds.md`](meds.md#auto-miss-engine-feature-040) — the auto-miss engine that consumes `intakeWindow`
- [`meds.md`](meds.md#today-screen--intake-logging-feature-038) — the Today screen that consumes all three intake settings
