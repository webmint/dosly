...
.
├── lib/
│   ├── main.dart                  # App entry point — runs ProviderScope + DoslyApp
│   ├── core/                      # Shared: theme, errors, constants, utilities
│   │   ├── error/                 # Failure classes for Either<Failure, T>
│   │   ├── theme/                 # ThemeData, color scheme, typography
│   │   ├── routing/               # GoRouter / Navigator config (cross-feature)
│   │   └── utils/                 # Shared helpers
│   └── features/                  # Feature modules — one folder per feature
│       └── [feature]/
│           ├── data/
│           │   ├── datasources/   # Remote (REST/Dio) and local (sqflite) sources
│           │   ├── models/        # JSON-serializable DTOs
│           │   └── repositories/  # Repository implementations (return Either)
│           ├── domain/
│           │   ├── entities/      # Pure Dart business objects (no Flutter imports)
│           │   ├── repositories/  # Abstract repository contracts
│           │   └── usecases/      # Single-purpose business operations
│           └── presentation/
│               ├── providers/     # Riverpod NotifierProviders / AsyncNotifiers
...
└── analysis_options.yaml          # dart analyze rules (flutter_lints + custom)
```

> This is the **target structure** for this greenfield project. Run `flutter create .` to scaffold the baseline, then create `lib/core/` and `lib/features/` per the layout above.

## Development Commands

<!-- defaults — update after `flutter create .` runs -->
...
- `dart analyze` — Static analysis: linting + type checking (single source of truth)
...
**API Layer**: REST (Dio or http package, TBD during /constitute)
...
3. Post-execution verification (dart analyze, build, done conditions, self-repair)
...
### `/verify [spec-file]`
Verifies all completed tasks against the spec's acceptance criteria. AC verification is set to **off** for this project (mobile app — verified by reading code and running `flutter test`). Incorporates `/review` findings if available (warns if missing). Performs cross-task integration check (not full code review — that was done per-task). Updates memory with lessons learned.
...
4. Verification (dart analyze, build, self-repair loop)
...
### `/refactor path/to/file.dart "goal"`
Focused code refactoring workflow for behavior-preserving restructuring (1-5 files). Supports IDE-injected context (active file/selection from Android Studio / IntelliJ / VS Code) or manual file path. Phases:
...
3. Apply refactoring with auto-selected agent (architect, mobile-engineer based on file layer)
4. Verification (dart analyze, build, tests, self-repair loop)
5. Code review (code-reviewer agent)
...
- `runtime-debugger` — Runtime error diagnosis and minimal-fix loop (Flutter DevTools, logs)
- `tech-writer` — Inline dartdoc and `docs/` folder maintenance
- `security-reviewer` — OWASP MASVS, secret leaks, insecure storage, mobile auth flows
- `mobile-engineer` — Flutter screens, navigation, platform channels, iOS/Android integration
- `architect` — Clean Architecture decisions, use cases, repositories, domain modeling
- `design-auditor` — Material 3 / Cupertino compliance, accessibility, responsive layouts
- `performance-analyst` — Startup time, 60fps frame budget, app size, memory profiling
...
- **PostToolUse hook**: `dart analyze` runs automatically after every file Edit/Write (covers both linting and type checking via `analysis_options.yaml`)
- **Pre-flight check**: Constitution and memory review before each task
- **Post-execution**: dart analyze + flutter test + build on all changed files after each task
...
7. **Document new code** — all new public functions/classes must have dartdoc (`///`) comments
8. **Lint everything** — `dart analyze` must pass on all changed files before task completion
9. **Handle both paths** — every fallible operation must return `Either<Failure, T>` or use try/catch with typed exceptions
10. **Validate at boundaries** — validate external input (user input, API responses, deep links); trust internal code
11. **SOLID, DRY, KISS** — single responsibility, don't repeat logic 3+ times, keep it simple
12. **Search before building** — before writing anything generic/reusable, search the codebase for existing utilities, helpers, or widgets that already do it
...
2. **Never commit secrets** — no API keys, tokens, or credentials in code (use `--dart-define` or secure storage)
3. **Never commit debug artifacts** — no `print()`, `debugPrint()` (outside of intentional logger), `debugger()` left behind
...
7. **Never use `!` (null assertion) without certainty** — null safety errors are runtime bugs in disguise
8. **Never put Flutter imports in `domain/`** — the domain layer must be pure Dart (testable without Flutter)
