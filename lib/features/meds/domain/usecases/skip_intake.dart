/// The single business operation for skipping a scheduled dose.
///
/// [SkipIntake] mints a fresh [IntakeId] via the injected [IdGenerator],
/// stamps the confirmation time in UTC, assembles a SKIPPED [Intake] for the
...
/// Records a scheduled dose occurrence as [IntakeStatus.skipped].
///
/// Builds the [Intake] for the occurrence — a freshly generated [IntakeId],
/// the confirmation timestamp in UTC, and the `skipped` status — then delegates
/// persistence to [IntakeRepository.skip]. It exists as the single business
/// operation for opting out of a dose (constitution §2.1, "one operation per
/// class") so presentation never touches the repository directly (§4.1.1).
class SkipIntake {
  /// Creates a [SkipIntake] use case backed by [_repository] for persistence
  /// and [_idGenerator] for minting the new intake ID.
  const SkipIntake(this._repository, this._idGenerator);

  final IntakeRepository _repository;
  final IdGenerator _idGenerator;

  /// Skips the dose occurrence identified by [medicationId], [slotId], and
  /// [scheduledAt], recording the action at [now].
  ///
  ...
  Future<Either<Failure, Intake>> call({
    ...
    return _repository.skip(intake);
  }
}
